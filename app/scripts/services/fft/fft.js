...
angular.module('activityRecognition').service('Fft', function(Complex, FftUtil, BitTwiddle) {/*===========================================================================*\
  ...
  //     BitTwiddle = require('bit-BitTwiddle');
  ...
  return {
    ...
    fftInPlace: function(vector) {
      ...
      var trailingZeros = BitTwiddle.countTrailingZeros(N); //Once reversed, this will be leading zeros

      // Reverse bits
      for (var k = 0; k < N; k++) {
        var p = BitTwiddle.reverse(k) >>> (BitTwiddle.INT_BITS - trailingZeros);
        if (p > k) {
